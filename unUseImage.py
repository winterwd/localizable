...
# 可能检查出错的图片，需要特别留意下
ERROR_DESPATH = "./result/unUseImage/error.log"

# 解析结果存放的路径
WDESPATH = "./result/unUseImage/image.log"

# 项目中没有用到的图片
IMAGE_WDESPATH = "./result/unUseImage/images/"
...
# Assets.xcassets
def isimageset(filePath):
    return 'imageset' in filePath
...
def un_use_image(filePath):
    ...
    if isimageset(filePath):
        exts = os.path.splitext(filePath)
        result = filename(filePath).replace(exts[1], '')
        source_images[result] = filePath
        # pass
    elif re.search(r'\w(@2x){0,1}.(png|jpg|jpeg|gif)', filePath):
        exts = os.path.splitext(filePath)
...
def find_from_file(path):
    ...
    for aCompent in paths:
        ...
        if isInBlackList(aPath):
            ...
            continue
        if os.path.isdir(aPath) and not isimageset(aPath):
            find_from_file(aPath)
        elif isimage(aPath) and not isappicon(aPath) and not is_except_image(aPath):
            un_use_image(aPath)
...
def main():
    ...
    for item in set(source_images.keys()) - use_images:
        ...
        if ext == '.imageset':
            shutil.copytree(value, IMAGE_WDESPATH + item + ext)
        else:
